...
### Бэкенды (режимы работы)

- **Unbound (рекурсивный, с DNSSEC)** — включается при сборке с флагом `-tags=unbound` и при включенном CGO. Скрипт `install.sh` автоматически пытается собрать именно этот вариант и устанавливает `libunbound-dev`, а также генерирует trust anchor (`/etc/unbound/root.key`).
  - Ручная сборка: `CGO_ENABLED=1 go build -tags=unbound -o astracat-dns .`
  - Требуется: `libunbound` и заголовки (`libunbound-dev`).
- **Stub (форвардер)** — запасной без-CGO режим, пересылает запросы на публичный DNS (по умолчанию `9.9.9.9:53`). Используется автоматически, если сборка с Unbound недоступна.
- **Knot (kres) backend** — пока не реализован. Сборка с тегом `kres` включает заглушку, возвращающую NOTIMP. Планируется полноценная рекурсивная интеграция с Knot Resolver отдельно.
