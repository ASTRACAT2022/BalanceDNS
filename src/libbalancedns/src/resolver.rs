...
impl ResolverCore {
    pub fn spawn(balancedns_context: &BalanceDNSContext) -> io::Result<Sender<ClientQuery>> {
        let config = &balancedns_context.config;
        let net_udp_socket = balancedns_context
            .udp_socket
            .try_clone()
            .expect("Unable to clone the UDP listening socket");
        let (resolver_tx, resolver_rx): (Sender<ClientQuery>, Receiver<ClientQuery>) =
            channel(balancedns_context.config.max_active_queries);
        ...
        let config = balancedns_context.config.clone();
        let dnstap_sender = balancedns_context.dnstap_sender.clone();
        let cache = balancedns_context.cache.clone();
        let varz = balancedns_context.varz.clone();
