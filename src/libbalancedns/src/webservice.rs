...
impl WebService {
    fn new(balancedns_context: &BalanceDNSContext) -> WebService {
        WebService {
            varz: balancedns_context.varz.clone(),
        }
    }
    ...
    pub fn spawn(
        ...
        let listen_addr = balancedns_context
            .config
            .webservice_listen_addr
            .parse()
            .expect("Unsupport listen address for the prometheus service");
        let web_service = WebService::new(balancedns_context);
