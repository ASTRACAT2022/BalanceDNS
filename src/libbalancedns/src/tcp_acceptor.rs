...
impl TcpAcceptorCore {
    ...
    pub fn spawn(
        balancedns_context: &BalanceDNSContext,
        resolver_tx: Sender<ClientQuery>,
        service_ready_tx: mpsc::SyncSender<u8>,
    ) -> io::Result<thread::JoinHandle<()>> {
        let net_tcp_listener = balancedns_context.tcp_listener.try_clone()?;
        let cache = balancedns_context.cache.clone();
        let varz = balancedns_context.varz.clone();
        let tcp_arbitrator = balancedns_context.tcp_arbitrator.clone();
        let timer = wheel()
