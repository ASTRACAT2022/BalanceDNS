...
pub fn question(packet: &[u8]) -> Result<QuestionRR<'_>, &'static str> {
    let packet_len = packet.len();
    if packet_len <= DNS_OFFSET_QUESTION {
...
pub fn min_ttl(
    ...
    for _ in 0..rrcount {
        ...
        let qtype = ((packet[offset] as u16) << 8) | packet[offset + 1] as u16;
        let qclass = ((packet[offset + 2] as u16) << 8) | packet[offset + 3] as u16;
        let ttl = ((packet[offset + 4] as u32) << 24)
            | ((packet[offset + 5] as u32) << 16)
            | ((packet[offset + 6] as u32) << 8)
            | packet[offset + 7] as u32;
...
pub fn build_query_packet(
    ...
    if (force_dnssec || normalized_question.dnssec) && qname_len > 0 {
...
pub fn qname_to_fqdn(qname: &[u8]) -> Result<String, &'static str> {
    let mut name = String::new();
    let mut offset = 0usize;
    while offset < qname.len() {
        let label_len = qname[offset] as usize;
        if label_len == 0 {
            break;
        }
        if label_len & 0xc0 == 0xc0 {
            return Err("Compressed labels are not supported here");
        }
        offset += 1;
        if offset + label_len > qname.len() {
            return Err("Malformed qname");
        }
        if !name.is_empty() {
            name.push('.');
        }
...
pub fn build_address_packet(
    ...
    let (rr_type, rdata) = match ip_addr {
        IpAddr::V4(addr) => {
            if normalized_question.qtype != DNS_TYPE_A {
                return build_nxdomain_packet(normalized_question);
            }
            (DNS_TYPE_A, addr.octets().to_vec())
        }
        IpAddr::V6(addr) => {
            if normalized_question.qtype != DNS_TYPE_AAAA {
                return build_nxdomain_packet(normalized_question);
            }
            (DNS_TYPE_AAAA, addr.octets().to_vec())
        }
    };
    let rdata_len = rdata.len();
    let capacity = DNS_HEADER_SIZE + normalized_question.qname.len() + 1 + 32 + rdata_len;
    let mut packet = Vec::with_capacity(capacity);
    packet.extend_from_slice(&[0u8; DNS_HEADER_SIZE]);
    set_tid(&mut packet, normalized_question.tid);
    set_aa(&mut packet, true);
    ...
    packet.push((rr_type >> 8) as u8);
    packet.push(rr_type as u8);
    packet.push((normalized_question.qclass >> 8) as u8);
    packet.push(normalized_question.qclass as u8);
    packet.push((ttl >> 24) as u8);
    packet.push((ttl >> 16) as u8);
