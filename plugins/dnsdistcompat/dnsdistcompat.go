...
func (p *Plugin) SetConfig(config map[string]any) error {
	...
	if v, ok := config["dropSuffixes"].([]string); ok {
		p.cfg.DropSuffixes = append([]string(nil), v...)
	}
	if v, ok := config["lateDropSuffixes"].([]string); ok {
		p.cfg.LateDropSuffixes = append([]string(nil), v...)
	}
	p.reloadLocked()
	p.mu.Unlock()
	return nil
