...
func (p *AdBlockPlugin) getUpdateInterval() time.Duration {
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (p *AdBlockPlugin) SetConfig(config map[string]any) error {
	...
	switch blocklists := config["blocklists"].(type) {
	case string:
		p.blocklists = splitLines(blocklists)
	case []string:
		p.blocklists = append([]string(nil), blocklists...)
	case []any:
		parsed := make([]string, 0, len(blocklists))
	...
	switch updateInterval := config["updateInterval"].(type) {
	case string:
