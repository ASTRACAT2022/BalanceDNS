...
func (p *HostsPlugin) getUpdateInterval() time.Duration {
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (p *HostsPlugin) SetConfig(config map[string]any) error {
	...
	if interval, ok := config["interval"].(string); ok && strings.TrimSpace(interval) != "" {
	...
	if interval, ok := config["updateInterval"].(string); ok && strings.TrimSpace(interval) != "" {
		if duration, err := time.ParseDuration(interval); err == nil {
			p.updateInterval = duration
		} else {
			p.mu.Unlock()
